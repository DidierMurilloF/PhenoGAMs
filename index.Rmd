---
title: 'Smoothers in the Field: GAMs for Capturing Treatment Effects and Spatial Trends
  in Plant Breeding Field Trials'
author: "Didier Murillo"
date: "2025-05-03"
output:
  distill::distill_article:
    highlight: kate  # or choose 'tango', 'espresso', etc.
---
...
The dataset originates from a partially replicated (p-rep) field trial design involving 180 genotypes (treatments) evaluated across five environments. For the purposes of this article, we focus on a single environment to illustrate a within-location spatial analysis. The data include field layout information (row and column positions), genotype identifiers (treatments), and grain yield as the primary response variable.
...
To extract genotype-level predictions from the model, we predict treatment effects while fixing all spatial covariates at their field-centered values. The way we created the newdata set for GAM model prediction is showing below:
...
Once the ASReml model is fitted, extracting the predicted genotype is straightforward using the `predict()` function. We augment the predictions with 95% confidence intervals based on the standard errors:
...
To assess how well each model captures spatial variation and explains yield variability, we begin by examining standard diagnostic plots. These include residual histograms, residuals versus fitted values, and QQ-plots. We start with the Generalized Additive Model (GAM) fitted in the previous section.
...
The diagnostic plots for both the GAM and ASReml models show some clear limitations. In the residuals vs. fitted values plots, we observe clustering of residuals, which likely reflect residual correlation among spatially adjacent plots. This pattern suggests that the current models have not fully accounted for spatial dependence in the errors. The QQ plots show deviations from normality, especially in the tails, and the histograms reveal slight skewness. These patterns point to remaining spatial variation or structure not yet modeled, indicating room for improvement in both approaches.
...
- $f_6$ is the tensor product with random effects (bs = "re") for both factors.

The extended GAM model is specified as follows:
...
The updated GAM model predictions are summarized below.
...
The new fashion Linear Mixed model predictions are now:
...
Let us check the new models result diagnostics
...
Together, these results suggest that incorporating spatial structure (via splines in ASReml and tensor product + AR1 in GAM) significantly enhanced the models' ability to capture underlying field variability. Both models now provide a much more reliable basis for estimating treatment effects in this breeding trial.
...
These heatmaps show the spatial distribution of residuals across the field layout. The colors range from blue (negative residuals) to yellow (positive residuals), helping us quickly identify areas of under- or over-estimation.

GAM model Residuals: The residuals are mostly centered around zero with just a few isolated high values, indicating that the model captures the spatial pattern well overall.

ASReml Residuals: Similarly, the ASReml model shows good residual control with a few small clusters, suggesting that the spatial structure was effectively modeled.

Both models show strong spatial fit, and these plots help confirm the lack of major residual patterns.
...
The Kendall's tau value for the full set of genotypes is high, indicating strong concordance in rankings between GAM and ASReml.
...
We can visualize the rank agreement below. Each point represents a genotype, and the dashed line marks perfect concordance. The plot confirms that rankings from GAM and ASReml are highly aligned:
...
We also examine the standard errors of the predicted treatment means. Comparing these across models provides insight into how uncertainty is estimated. Despite their different formulations, both GAM and ASReml produce strikingly similar standard errors across genotypes:
...
It is reassuring to see that the uncertainty associated with genotype means is consistently estimated across both models. This further supports the robustness of GAMs as a complementary tool for spatial modeling in plant breeding field trials.
...
The full code used in this analysis is available on GitHub: github.com/DidierMurilloF/PhenoGams

### References

This work draws inspiration from _Generalized Additive Models: An Introduction with R (2nd Edition)_ by Simon Wood, an accessible and practical guide to understanding and implementing GAMs using the `mgcv` package.


### About the Author

**Didier Murillo** is a statistician and R Shiny developer specializing in agricultural data analytics at North Dakota State University (NDSU). His research focuses on spatial modeling, predictive pipelines, and the integration of genomic and phenotypic data in plant breeding programs. He is also an expert in developing robust R Shiny applications that integrate analytical pipelines into user-friendly tools for researchers, breeders, and other stakeholders.

Connect with Didier on [![GitHub Repo](https://img.shields.io/badge/GitHub-PhenoGams-blue?logo=github)](https://github.com/DidierMurilloF/PhenoGams)
or [LinkedIn](https://www.linkedin.com/in/damurillof/).
