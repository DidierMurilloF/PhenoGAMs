...
To extract genotype-level predictions from the model, we predict treatment effects while fixing all spatial covariates at their field-centered values.
...
```{r asreml_model1, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
suppressMessages({
  suppressWarnings({
			asreml_m1 <- asreml(
				fixed = YIELD ~ ROW + COLUMN,
...
Once the ASReml model is fitted, extracting the predicted genotype is straightforward using the `predict()` function. 

```{r}
pred_asreml <- predict(asreml_m1, classify = "TREATMENT", sed = TRUE)$pvals
```

We augment the predictions with 95% confidence intervals based on the standard errors:

```{r, eval=TRUE, echo=FALSE}
pred_asreml |> add_confidence_intervals(pred_col = predicted.value, se_col = std.error) |> head(10) |> kable()
```
...
- $f_6$ is the tensor product with random effect (bs = "re")
...
BLUPs_gam_single2 |> head(10) |> kable()
```
...
```{r asreml_model12, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
suppressMessages({
  suppressWarnings({
			asreml_m2 <- asreml(
				fixed = YIELD ~ ROW + COLUMN,
...
The Mixed model predictions are now 

```{r}
pred_asreml2 |> add_confidence_intervals(pred_col = predicted.value, se_col = std.error) |> head(10) |> kable()
```
...
```{r, fig.align="c", fig.width=12, fig.height=6, echo=FALSE, eval=TRUE}
